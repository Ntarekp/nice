...
export default function InspectionsPage() {
  ...
  const scheduleMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries(["inspections"]); setShowForm(false); toast.success("Inspection scheduled!"); },
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
        <div>
          <h1 style={{ fontSize: "1.4rem", fontWeight: 600 }}>🔍 Inspections</h1>
          <p style={{ color: "var(--color-text-muted)", fontSize: "0.85rem" }}>{data?.pagination?.total ?? 0} total</p>
        </div>
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr key={ins.id} style={{ borderTop: "1px solid var(--color-border)", fontSize: "0.875rem" }}>
                ...
                <td style={{ padding: "0.75rem 1rem" }}>
                  {ins.status === "scheduled" && ["admin","inspector"].includes(user?.role) && (
      ...
      {showForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}
          ...
          <div style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: "16px", padding: "2rem", width: "100%", maxWidth: "460px" }}
            ...
            <h3 style={{ fontWeight: 600, marginBottom: "1.5rem" }}>Schedule Inspection</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {[["extinguisherId","Extinguisher UUID","text"],["scheduledDate","Date & Time","datetime-local"]].map(([k,l,t]) => (
